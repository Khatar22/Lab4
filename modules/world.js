...
export class World {
  ...
  tryEncounter(tile, chance = 0.2) {
    ...
    // 4) Фильтрация некорректных шансов
    ...
    let sum = 0;
    let chosen = valid[valid.length - 1]; // фолбэк — последний покемон
    ...
    // 6) Безопасное вычисление уровня
    const base = (this.player && this.player.active && typeof this.player.active.level === 'number')
      ? this.player.active.level
      : 5; // запасной вариант
